...
-- 需要被删除的文件路径
create table
    if not exists file_to_del_table (file_path text primary key);

-- 引用计数为0时自动删除，并将文件路径插入到 file_to_del_table
create trigger if not exists del_ref_when_count_zero after
...
create table
    if not exists shared_link_table (
...
-- 剩余访问次数为0后，删除link
create trigger if not exists del_shared_if_left_times_zero after
...
create trigger if not exists inc_ref_when_ins_entry after insert on entry_table for each row when NEW.ref_id != 0 begin
...
create trigger if not exists del_shared_when_del_entry after delete on entry_table for each row when OLD.shared_link != '' begin
delete from shared_link_table
where
...
create trigger if not exists dec_ref_when_del_entry after delete on entry_table for each row when OLD.ref_id != 0 begin
update file_ref_table
set
    ref_count = ref_count - 1
where
...
create trigger if not exists del_rel_when_del_entry after delete on entry_table for each row when OLD.ref_id = 0 begin
